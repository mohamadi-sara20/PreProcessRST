...
def binarize(span_node):
    ...
    # handle 3 childern
    new_nodes = []
    multinuc_found = False
    for i in range(len(span_node.children)):
        if span_node.children[i].multinuc == 'c' and not multinuc_found:
            if i == 2:
                raise Exception(f'SINGLE MULTINUC FOUND, NODE ID: {span_node.node_id}')
            if span_node.children[i+1].multinuc != 'c' or span_node.children[i].rel != span_node.children[i+1].rel:
                raise Exception(f'Multinuc Node Error! Multinuc Relation in Non-Adjacent Nodes; node id is: {span_node.node_id}')
            intermediate_node = Node(left=span_node.children[i], right=span_node.children[i+1], node_id='-10', rel='span')
            new_nodes.append(intermediate_node)
            multinuc_found = True
            if i == 0:
                new_nodes.append(span_node.children[2])
                break
        else:
            new_nodes.append(span_node.children[i])
    ...
    if multinuc_found:
        ...
        intermediate_node = Node(left=span_node.children[0], right=span_node.children[1], node_id='-1', rel='span')
...
if __name__ == '__main__':
    ...
    with open('maz.txt') as f:
        text = f.read()
    tokens = nlp(text)
    out_node, leaf_idx = xml2tree('maz.rs3',nlp)
    binarize(out_node)
