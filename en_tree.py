...
def xml2tree(fname, tokens, nlp):
    ...
    for node in nodes:
        ...
        if 'parent' not in node:
            ...
            lf_idx = sorted(leaf_idx, key=lambda x: float(x))
    ...
    gg = Node(None,None,'-10')
    gg.left = node_dict[root_ind]
    return gg, leaf_idx
...
def rebuild_tree(tree_node):
    ...
    # mononuc nodes
        ...
        if tree_node.left is not None and tree_node.right is not None and (
            ...
            right_child = Node(left=None, right=None, node_id=tree_node.node_id, rel='leaf', is_leaf=True, multinuc='t', leaf_range=tree_node.leaf_range)
        ...
        # leaf - span
            ...
            right_child = Node(left=tree_node.left.left, right=tree_node.left.right, node_id=tree_node.left.node_id, rel='span', leaf_range=tree_node.left.leaf_range, is_leaf=tree_node.left.is_leaf, multinuc=tree_node.left.multinuc)
            if tree_node.node_id < tree_node.left.node_id:
                tree_node.left = left_child
                tree_node.right = right_child
            else:
        ...
        # span - leaf
            ...
            left_child = Node(left=tree_node.left.left.left, right=tree_node.left.left.right, node_id=tree_node.left.left.node_id,
                               rel='span', is_leaf=tree_node.left.left.is_leaf, multinuc=tree_node.left.left.multinuc, leaf_range= tree_node.left.left.leaf_range)
