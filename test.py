    ...
    for node in nodes:
        if 'parent' not in node:
            ...
            if idx in leaf_idx:
                ...
                #             if i > 0:
                #                 begin += len(tokens.sentences[i-1].words)
                #                 end += len(tokens.sentences[i-1].words)
                #             else:
                #                 begin = 0
                #                 end = 0
                #             if ln_text in tokens.sentences[i].text:
                #                 ln_text_processed = nlp(ln_text)
                #                 if len(ln_text_processed.sentences) == 1:
                #                     s = begin + ln_text_processed.sentences[0].words[0].id - 1
                #                     e = end + ln_text_processed.sentences[0].words[-1].id - 1
                #                     node_dict[idx].leaf_range = str(s) + " " + str(e)
                #                 else:
                #                     print('############### length greater than one: ' + str(len(ln_text_processed.sentences)) + "########")
...
if __name__ == '__main__':
    ...
    tokens = nlp(text)
    out_node, leaf_idx = xml2tree('wsj_input_rs3.xml', tokens)
    ...
    output_tree = ''
    h = open('kkkkk', 'w')
    print(preoder_print(tree_rebuilt))
    h.close()
